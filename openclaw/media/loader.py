...
class MediaLoader:
    ...
    async def _load_data_url(self, data_url: str) -> MediaResult:
        ...
        if self.max_bytes and len(buffer) > self.max_bytes:
            raise ValueError(
                f"Data URL exceeds size limit: {len(buffer)} > {self.max_bytes}"
            )
    ...
    async def _load_file(self, file_path: str) -> MediaResult:
        ...
        if self.workspace_root:
            ...
            try:
                ...
                if fallback.exists():
                    ...
                    raise ValueError(
                        f"Path outside workspace: {path} (workspace: {workspace})"
                    )
        ...
        if self.max_bytes and len(buffer) > self.max_bytes:
            raise ValueError(
                f"File exceeds size limit: {len(buffer)} > {self.max_bytes} ({path})"
            )
