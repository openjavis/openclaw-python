    ...
    def _create_message_handler(self, channel_id: str) -> MessageHandler:
        ...
        async def handler(message: InboundMessage) -> None:
            ...
            try:
                ...
                images = None
                if message.metadata and "images" in message.metadata:
                    images = message.metadata["images"]
                    if images:
                        logger.info(
                            f"📥 [{channel_id}] Received {len(images)} image(s) with message"
                        )

                # Process through Agent Runtime
                response_text = ""

                tools = env.tools if env else []
                async for event in runtime.run_turn(
                    session, message.text, tools=tools, images=images
                ):
